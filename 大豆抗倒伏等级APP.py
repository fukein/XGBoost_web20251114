...
# # 加载XGBoost模型
# try:
#     model = joblib.load('XGBoost.pkl')  # 加载指定模型文件
...
#         3: ("purple", "极高")
#     }
#     color, level_text = class_colors.get(int(res["pred_class"]), ("blue", "未知"))
    
#     st.markdown(
#         f"<div style='font-family: Times New Roman; font-size:16px; color:{color}; font-weight:bold; margin:12px 0'>"
#         f"预测倒伏级别: {int(res['pred_class'])} ({level_text}风险)<br>"
#         f"各级别概率: {[f'级别{i}: {p:.2%}' for i, p in enumerate(res['pred_proba'])]}"
#         "</div>",
#         unsafe_allow_html=True
#     )
...
#             values=res['single_shap'],  # 现在已经是单个样本的SHAP值了
#             base_values=res['base_value'],
#             data=res['feature_values'],  # 单个样本的特征值
